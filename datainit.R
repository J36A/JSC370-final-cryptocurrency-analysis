...
max_volume <- "HIVsRcGKkPFtW"
max_price <- "xz24e0BjL"
min_change <- "3mVx2FX_iJFp5"
min_volume <- "65PHZTpmE55b"
min_price <- "SbWqqTui-"

database <- database %>% filter(ids %in% c(bitcoin, max_volume, max_price, min_change, min_volume, min_price))

query_string <- list(timePeriod = "5y") # Extract the last five years data

price_url <- paste("https://coinranking1.p.rapidapi.com/coin/", bitcoin, "/history", sep="")
